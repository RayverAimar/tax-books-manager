 ...
 */
export const RecentPeriodsCard: React.FC<RecentPeriodsCardProps> = ({
  companyId,
  onPeriodClick
}) => {
  ...
  useEffect(() => {
    const loadRecentPeriods = async () => {
      try {
        ...
        const recentSales = salesPeriods
          .filter(p => p.hasData)
          .slice(0, 3);

        const recentPurchases = purchasesPeriods
          .filter(p => p.hasData)
          .slice(0, 3);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Periodos Recientes</CardTitle>
        <CardDescription>
          Últimos periodos con información registrada
        </CardDescription>
      </CardHeader>
      <CardContent>
        {!hasAnyData ? (
          <div className="text-center py-8">
            ...
            <p className="mt-4 text-sm text-muted-foreground">
              No hay periodos con datos registrados
            </p>
            <p className="mt-1 text-xs text-muted-foreground">
              Comienza importando o agregando registros
            </p>
          </div>
        ) : (
